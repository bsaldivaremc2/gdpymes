...
def introducir_ruido(df, perc_nulos, perc_outliers, columnas_numericas):
    ...
    for col in df_ruido.columns:
        # Seleccionamos aleatoriamente índices para convertirlos en nulos
    ...
    for col in columnas_numericas:
        ...
        # Multiplicamos por valores muy altos o negativos para crear outliers obvios
        factores = np.random.choice([10, 50, -10, -50], size=mask_outliers.sum())
        # Nos aseguramos de no operar sobre nulos creados en el paso anterior
        valid_mask = mask_outliers & df_ruido[col].notna()
        # Aseguramos de tener suficientes factores para la valid_mask
...
# Caso 1: Andrés (Ingeniería civil / Conservación de carreteras)
def generar_caso1_carreteras():
    data = {
        'id_tramo': [f"TR-{i}" for i in range(NUM_ROWS)],
        'trafico_diario': np.random.randint(500, 20000, NUM_ROWS),
        'indice_desgaste_firme': np.random.uniform(1.0, 10.0, NUM_ROWS).round(2), # 10 es perfecto, 1 es destruido
...
# Caso 2: Javier (Redes sociales / Divulgación científica)
def generar_caso2_rrss():
    ...
    # Hacer que los likes tengan correlación con las visualizaciones
...
# Caso 3: Carlos (TI empresarial / Visión artificial estructuras metálicas)
...
# Caso 4: Amparo (Programación Web / Analítica ML)
def generar_caso4_web():
    data = {
        ...
        'convirtio_compra': np.random.choice([1, 0], NUM_ROWS, p=[0.08, 0.92]) # 1: Sí, 0: No
    }
    return pd.DataFrame(data), ['paginas_vistas', 'tiempo_en_sitio_seg', 'convirtio_compra']

# Caso 5: Josep (Máquinas recreativas)
def generar_caso5_recreativas():
    ...
    # Evitamos recaudaciones negativas en datos limpios
    df = pd.DataFrame(data)
...
# Caso 6: Fernando (Baterías industriales y de tracción)
def generar_caso6_baterias():
    ...
    # Baterías con más ciclos deberían tener menor SOH
    df = pd.DataFrame(data)
...
# Caso 7: Juanjo (Reparaciones aseguradoras)
def generar_caso7_aseguradoras():
    ...
    df = pd.DataFrame(data)
    # Generamos un coste real que suele ser parecido al estimado pero con variaciones
...
# ==========================================
# 3. GENERACIÓN Y EXPORTACIÓN DE LOS CSVs
# ==========================================
...
for nombre_caso, funcion_generadora in casos.items():
    ...
    nombre_limpio = f"{nombre_caso}_1_Limpio.csv"
    df_limpio.to_csv(nombre_limpio, index=False)
    ...
    nombre_ligero = f"{nombre_caso}_2_ErroresLigeros.csv"
    df_errores_ligeros.to_csv(nombre_ligero, index=False)
    ...
    nombre_grave = f"{nombre_caso}_3_ErroresGraves.csv"
    df_errores_graves.to_csv(nombre_grave, index=False)
    
    print(f"✅ Generados los 3 archivos para: {nombre_caso}")

print("\n¡Proceso finalizado! Los 21 archivos CSV están disponibles en la pestaña 'Archivos' de Colab.")
